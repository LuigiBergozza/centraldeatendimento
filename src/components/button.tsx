...
import { twMerge } from "tailwind-merge"; // É bom usar twMerge se você estiver mesclando classes
...
const variants = tv({
  base: "relative border-2 shadow-md w-full text-center flex items-center justify-center px-5 py-1.5 outline-none cursor-pointer rounded-md text-secondary not-disabled:hover:bg-primary not-disabled:hover:border-0 rounded-full font-medium duration-500 not-disabled:active:scale-95 disabled:opacity-45 disabled:cursor-not-allowed",
  ...
  variants: {
    variant: {
      ...
      Menu: "h-13 w-13 border-0 md:h-16 md:w-16 2md:h-20 2md:w-20 p-0 bg-priDark not-disabled:hover:bg-tertiary",
    },
  },
});
...
export const Button = ({ children, variant, className, ...props }: Props) => {
  ...
  // Lógica para renderização condicional da "bolinha"
  const shouldShowDot = variant !== "Menu";

  return (
    // Utilize twMerge para combinar as classes geradas pelo variants com quaisquer classes passadas via 'className' prop
    <button className={twMerge(variants({ variant }), className)} {...props}>
      {children}
      {/* Condição: A bolinha (<span>) só será renderizada se a variante NÃO for "Menu" */}
      {shouldShowDot && (
        <span className="absolute right-2 text-tertiary mr-2 text-2xl">•</span>
      )}
    </button>
  );
};